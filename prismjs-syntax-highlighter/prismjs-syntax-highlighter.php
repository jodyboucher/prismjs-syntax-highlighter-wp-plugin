 ...
 * @copyright   Copyright (c) 2016 Jody Boucher
 *
 *
...
class prismjs_syntax_highlighter
{
    ...
    public function __construct()
    {
        ...
        if (is_admin()) {
            register_activation_hook(__FILE__, array('prismjs_syntax_highlighter', 'on_activation'));
    ...
    function register_admin_init()
    {
        ...
        if (!isset($typenow) && !in_array($typenow, array('post', 'page'))) {
            debug_log('-- not post');
            return;
        }
    ...
    public function register_admin_notices()
    {
        ...
        if (isset($options[self::OPTION_SHOW_CSS_WARNING_NOTICE]) && $options[self::OPTION_SHOW_CSS_WARNING_NOTICE] === '1') {
            ?>
            <div class="updated">
                ...
                <p>
                    <strong>Please <a href="<?php echo admin_url() . 'theme-editor.php'; ?>">edit the theme</a> and
                        comment out or remove the relevant lines.</strong>
                </p>

                <form method="post" action="options.php">
                    <?php settings_fields('prismjs-settings-group'); ?>

                    <input type="hidden" id="<?php echo self::OPTION_SHOW_CSS_WARNING_NOTICE; ?>"
                           name="<?php echo $setting_name; ?>" value="0"/>
    ...
    public function register_admin_scripts()
    {
        ...
        echo
            '<script type="text/javascript">
                var defaultLanguage = "' . ArrayHelper::GetValueOrDefault($options, self::OPTION_DEFAULT_LANGUAGE, '') . '";
				var defaultInline = ' . (ArrayHelper::GetValueOrDefault($options, self::OPTION_DEFAULT_INLINE, '') === '1' ? 'true' : 'false') . ';
				var defaultLineNumbers = ' . (ArrayHelper::GetValueOrDefault($options, self::OPTION_DEFAULT_LINE_NUMBERS, '') === '1' ? 'true' : 'false') . ';
			</script>';
    ...
    function register_options_page()
    {
        ...
        add_options_page('Prism.js Syntax Highlighter', 'Prism.js', 'manage_options', 'prismjs-syntax-highlighter', array($this, 'options_page'));
    ...
    public function setting_custom_css()
    {
        ...
        $html = '<input type="text" id="' . self::OPTION_CUSTOM_CSS . '" name="' . $setting_name . '"';
        $html .= ' value="' . ($option_value != '' ? esc_attr($option_value) : '') . '" />';
        $html .= '<p class="description">File must be in ' . plugin_dir_path(__FILE__) . 'css/' . '</p>';
        debug_log('html to output: ' . $html);
    ...
    public function setting_custom_js()
    {
        ...
        $html = '<input type="text" id="' . self::OPTION_CUSTOM_JS . '" name="' . $setting_name . '"';
        $html .= 'value="' . ($option_value != '' ? esc_attr($option_value) : '') . '" />';
        $html .= '<p class="description">File must be in ' . plugin_dir_path(__FILE__) . 'js/' . '</p>';
        debug_log('html to output: ' . $html);
    ...
    public function setting_default_inline()
    {
        ...
        // get the current options values
        $options = get_option(self::SETTINGS_GROUP);
        $option_value = $options[self::OPTION_DEFAULT_INLINE];
        debug_log('current value of ' . self::OPTION_CUSTOM_JS . ': ' . $option_value);

        $setting_name = self::SETTINGS_GROUP . '[' . self::OPTION_DEFAULT_INLINE . ']';

        $html = '<input type="checkbox" id="' . self::OPTION_DEFAULT_INLINE . '"';
        $html .= ' name="' . $setting_name . '" value="1" ' . checked(1, $option_value, false) . '/>';
        debug_log('html to output: ' . $html);
    ...
    public function setting_default_language()
    {
        ...
        $js_file_contents = file_get_contents(plugin_dir_path(__FILE__) . 'js/' . $js_file);
        $languages = null;
        ...
        $html = '<select id="' . self::OPTION_DEFAULT_LANGUAGE . '" name="' . $setting_name . '"';
        foreach ($languages[1] as $language) {
            $html .= '<option value="' . $language . '"' . selected($option_value, $language, false) . '>' . $language . ' </option>';
        }
    ...
    public function setting_default_line_number()
    {
        ...
        // get the current options values
        $options = get_option(self::SETTINGS_GROUP);
        $option_value = $options[self::OPTION_DEFAULT_LINE_NUMBERS];
        ...
        $html = '<input type="checkbox" id="' . self::OPTION_DEFAULT_LINE_NUMBERS . '" name="' . $setting_name . '"';
        $html .= ' value="1" ' . checked(1, $option_value, false) . '/>';
        debug_log('html to output: ' . $html);
    ...
    public function setting_show_css_warning_notice()
    {
        ...
        $html = '<input type="hidden" id="' . self::OPTION_SHOW_CSS_WARNING_NOTICE . '" name="' . $setting_name . '"';
        $html .= ' value="' . ($option_value != '' ? esc_attr($option_value) : 'no') . '" />';
        debug_log('html to output: ' . $html);
    ...
    /**
     * Validate/Sanitize user inputed settings values.
     */
...
$prismjsSyntaxHighlighter = new prismjs_syntax_highlighter();
